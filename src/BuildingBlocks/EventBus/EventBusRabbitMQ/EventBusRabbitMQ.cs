...
public class EventBusRabbitMQ : IEventBus, IDisposable
{
    ...
    public EventBusRabbitMQ(IRabbitMQPersistentConnection persistentConnection, ILogger<EventBusRabbitMQ> logger,
        IServiceProvider serviceProvider, IEventBusSubscriptionManager subManager, string queueName = null,
        int retryCount = 5)
    {
        _persistentConnection = persistentConnection ?? throw new ArgumentNullException(nameof(persistentConnection));
        _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        _subManager = subManager ?? new InMemoryEventBusSubscriptionManager();
    ...
    {
       _logger.LogInformation("Subscribin to dynamic event {EventName} with {EventHandler}", eventName, typeof(TH).GetGenericTypeName());
        
        DoInternalSubscription(eventName);
        _subManager.AddDynmaicSubscription<TH>(eventName);
        StartBasicConsume();
    }
        ...
        where TH : IIntegrationEventHandler<T>
    {
        ...
        _logger.LogInformation("Subscribin to event {EventName} with {EventHandler}", eventName, typeof(TH).GetGenericTypeName());
    ...
    public void Dispose()
    {
        // if the consumer channel is not null, then dispose it
    ...
    private void SubManager_OnEventRemoved(object sender, string eventName)
    {
        ...
        // if there are no more subscriptions, then delete the queue
    ...
    private IModel CreateConsumerChannel()
    {
        ...
        // autoDelete: false - the queue won't be deleted once the channel is closed
    ...
    private void StartBasicConsume()
    {
        ...
        // autoAck: false - the consumer will acknowledge the message only after the event is processed
        if (_consumerChannel != null)
        {
        ...
        {
            _logger.LogError("Consumer channel is null");
        }
    }
    ...
    private async Task ProcessEvent(string eventName, string message)
    {
        ...
        // if there is a subscription for the eventName, then process the message
